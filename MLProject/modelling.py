...
def save_confusion_matrix(y_true, y_pred, title, path):
    ...
    ConfusionMatrixDisplay(confusion_matrix(y_true, y_pred),
                           display_labels=["No Disease", "Disease"]).plot(ax=ax, cmap="Blues")
...
def run_training(args, X_tv, y_tv, X_test, y_test, model):
    """Semua logging MLflow di sini — dipanggil di dalam konteks run yang sudah aktif."""
    ...
    with open(report_path, "w") as f:
        json.dump(classification_report(y_test, y_pred,
                  target_names=["No Disease", "Disease"], output_dict=True), f, indent=2)
    mlflow.log_artifact(report_path, "reports")

    signature = infer_signature(X_tv, model.predict(X_tv))
    mlflow.sklearn.log_model(model, artifact_path="model",
                             signature=signature, input_example=X_tv.head(3))
...
def main():
    ...
    if env_run_id:
        log.info(f"Using MLFLOW_RUN_ID from environment: {env_run_id}")
        with mlflow.start_run(run_id=env_run_id):
            run_training(args, X_tv, y_tv, X_test, y_test, model)
    else:
        log.info("No active run detected. Starting new run...")
